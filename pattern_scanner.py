        ...
        if any("ticker" in c for c in cols) or any("symbol" in c for c in cols):
            for col in df.columns:
                ...
                if "ticker" in low or "symbol" in low:
                    return _dedupe(df[col].dropna().astype(str).tolist())
    raise ValueError("Could not find a ticker column for Nasdaq-100")
...
def scan_oxabc_patterns(
    ...
    """Scan for completed and forming O-X-A-B-C patterns."""
